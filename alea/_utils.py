...
def read_neyman_threshold_update(
        file_name,
        threshold_key,
...
class VariationConvenience(object):
    ...
    def __init__(
            self,
            parameters_to_vary: dict, parameters_to_zip: dict,
    ...
    def _create_generate_args_input(self):
        ...
        for par in self.parameters_to_vary_in_generate_args.keys():
            for arg, value in zip(
                self.generate_args_to_zip, self.zip_input[par]):
        ...
        if "livetime" in combination.keys():
            ...
            if isinstance(combination["livetime"], list):
                for idx_lt, value in enumerate(combination["livetime"]):
                    ...
                    copy_of_merged_combinations[idx][
                        "livetime_" + str(idx_lt)] = value
            else:
...
def manipulate_toy_data(
    ...
    for idx, (data, ll) in enumerate(zip(data_sets, statistical_model.lls)):
        ...
        for cs1_range, popt in smearing_pars.items():
            ...
            modified_RG_and_RGX = sps.norm.rvs(
                loc=popt[1], scale=popt[2], size=nevents)
            modified_data["log10cs2"][data_mask] = modified_RG_and_RGX

            if plot:
                fig, ax = plt.subplots()
                title = f"cs1 range: {cs1_range}PE"
                ax.set_title(title)
                ax.scatter(
                    data[~data_mask]["cs1"],
                ...
                ax.axvline(
                    np.median(modified_RG_and_RGX),
                    label=label,
                    color="C0")
                ax.axvline(
                    np.median(data[data_mask]["log10cs2"]),
...
def _get_generate_args_from_toyfile(toydata_file):
    ...
    all_generate_args_equal = all(
        generate_args_list[0] == generate_args
