...
class TestSidebandConstraintConstruction(TestCase):
    ...
    def test_n_sideband_must_be_none_if_not_from_sideband(self):
        ...
        with self.assertRaisesRegex(ValueError, "n_sideband should only be set when from_sideband is True"):
            Parameter(name="tau", nominal_value=1.0, n_sideband=10)

    def test_no_uncertainty_when_from_sideband(self):
        """Providing an explicit uncertainty alongside from_sideband=True should raise a ValueError."""
        with self.assertRaisesRegex(ValueError, "uncertainty should not be provided"):
            Parameter(name="tau", nominal_value=1.0, from_sideband=True, n_sideband=10, uncertainty=0.1)
...
class TestSidebandConstraintConfig(TestCase):
    ...
    def test_sideband_set_type(self):
        """The from_sideband property should return a Parameters object containing only the sideband parameters."""
    ...
    def test_from_sideband_parameters_are_identified(self):
        """The from_sideband property should correctly identify which parameters are from sideband based on the config."""
        if self.params is None:
            self.skipTest("Skipping: config failed to load")
        from_sideband_params = self.params.from_sideband
    ...
    def test_from_sideband_parameters_have_n_sideband_set(self):
        """The from_sideband parameters should have n_sideband and uncertaintyset according to the config."""
        if self.params is None:
            self.skipTest("Skipping: config failed to load")
        from_sideband_params = self.params.from_sideband
